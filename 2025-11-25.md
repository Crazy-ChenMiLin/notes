...
### 方式 2：系统服务启动（推荐！开机自启 / 长期使用）

#### 1. 创建 redis.service 服务文件

通过 XFTP 在`/etc/systemd/system`目录创建`redis.service`文件，内容如下：
...
[Service]
Type=forking
# 启动命令（redis-server路径+配置文件实际路径）
ExecStart=/usr/local/bin/redis-server /usr/local/src/redis-6.2.6/redis.conf
# 停止命令（替换为你的Redis密码）
ExecStop=/usr/local/bin/redis-cli -h 127.0.0.1 -p 6379 auth 你的密码 shutdown
